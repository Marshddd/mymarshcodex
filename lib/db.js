...
async function ensureMongoSeed(db) {
  ...
  const currentLessonCount = courses.reduce((sum, course) => sum + (course.lessons?.length || 0), 0);
  const seedLessonCount = seed.courses?.reduce((sum, course) => sum + (course.lessons?.length || 0), 0) || 0;
  const currentQuestionCount = quizzes.reduce((sum, quiz) => sum + (quiz.questions?.length || 0), 0);
  const seedQuestionCount = seed.quizzes?.reduce((sum, quiz) => sum + (quiz.questions?.length || 0), 0) || 0;
